/**
 * Email Verification (OTP) Page — Premium design
 */
...
export default function VerifyEmail() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) { toast.success('Email verified successfully!'); navigate('/login'); }
      else toast.error(res.error?.message || 'Invalid verification code');
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (res.success) { toast.success('New code sent to your email'); setResendCooldown(60); }
      else toast.error(res.error?.message || 'Failed to resend code');
